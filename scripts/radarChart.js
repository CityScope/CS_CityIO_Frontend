...
export function RadarChart() {
      ...
      updateData = function() {
        ...
        chart_node
          ...
          .attr(
            ...
            "translate(" +
              ((options.width -
                (options.margins.left + options.margins.right)) /
                2 +
        ...
        legend_node.attr(
          "transform",
          "translate(" +
            options.legend.position.x +
            "," +
            options.legend.position.y +
            ")"
        );

        var update_gridCircles = axisGrid
        ...
        var update_axisLabels = axisGrid
          .selectAll(".axisLabel")
          .data(d3.range(1, options.circles.levels + 1).reverse());
        ...
        update_axisLabels
          ...
          .duration(duration / 2)
          .text(function(d, i) {
            if (radial_calcs.maxValue)
              return Format(
                (radial_calcs.maxValue * d) / options.circles.levels
              );
          })
          .attr("y", function(d) {
        ...
        var update_lines = update_axes.selectAll(".line").data(function(d) {
          return [d];
        }, get_axis);
        ...
        update_lines
          ...
          .attr("x2", function(d, i, j) {
            return calcX(null, 1.1, j);
          })
          .attr("y2", function(d, i, j) {
            return calcY(null, 1.1, j);
          })
        ...
        var update_axis_legends = update_axes
          .selectAll(".axis_legend")
          .data(function(d) {
            return [d];
          }, get_axis);
        ...
        var update_blobWrapper = chart_node
        ...
        var update_radarArea = update_blobWrapper
          .selectAll(".radarArea")
          .data(function(d) {
            return [d];
          }, get_key);
        ...
        update_radarArea
          ...
          .attr("class", function(d) {
            return "radarArea " + d.key.replace(/\s+/g, "");
          })
          .attr("d", function(d, i) {
          ...
          .on("mouseover", function(d, i) {
            if (events.radarArea.mouseover)
              events.radarArea.mouseover(d, i, this);
          })
          .on("mouseout", function(d, i) {
            if (events.radarArea.mouseout)
              events.radarArea.mouseout(d, i, this);
          });
        ...
        update_radarArea
          ...
          .style("fill-opacity", function(d, i) {
            return options.areas.filter.indexOf(d.key) >= 0
              ? 0
              : options.areas.opacity;
          });

        var update_radarStroke = update_blobWrapper
        ...
        let update_radarCircle = update_blobWrapper
          .selectAll(".radarCircle")
          .data(function(d, i) {
            return add_index(d._i, d.values);
          });
        ...
        update_radarCircle
          ...
          .style("fill", function(d, i, j) {
            return setColor(d, d._i, _data[j].key);
          })
          .style("fill-opacity", function(d, i) {
            return 0;
          })
          ...
          .duration(duration)
          .attr("cx", function(d, i) {
            return calcX(d.value, 0, i);
          })
          .attr("cy", function(d, i) {
            return calcY(d.value, 0, i);
          });
        ...
        var update_blobCircleWrapper = hover_node
        ...
        let update_radarInvisibleCircle = update_blobCircleWrapper
        ...
        update_radarInvisibleCircle
          ...
          .on("mouseover", function(d, i) {
            if (events.radarInvisibleCircle.mouseover)
              events.radarInvisibleCircle.mouseover(d, i, this);
          })
          .on("mouseout", function(d, i) {
            if (events.radarInvisibleCircle.mouseout)
              events.radarInvisibleCircle.mouseout(d, i, this);
          });
        ...
        if (options.legend.display) {
          ...
          if (d3.legend) {
            var legendOrdinal = d3.legend
              ...
              .scale(colorScale)
              .labels(
                colorScale.domain().map(function(m) {
                  return keyScale(m);
                })
              )
              .on("cellclick", function(d, i) {
                if (events.legend.mouseclick)
                  events.legend.mouseclick(d, i, this);
              })
              .on("cellover", function(d, i) {
                if (events.legend.mouseover)
                  events.legend.mouseover(d, i, this);
              })
              .on("cellout", function(d, i) {
                if (events.legend.mouseout) events.legend.mouseout(d, i, this);
              });

            legend_node.call(legendOrdinal);
          }
        }
      };
    });
  ...
  function radialCalcs() {
    ...
    radial_calcs = {
      ...
      radius: Math.min(
        (options.width - (options.margins.left + options.margins.right)) / 2,
      ...
      maxValue: Math.max(
        options.circles.maxValue,
        d3.max(_data, function(i) {
          return d3.max(
            i.values.map(function(o) {
              return o.value;
            })
          );
        })
  ...
  function calcX(value, scale, index) {
    return (
      radial_calcs.rScale(value ? value : radial_calcs.maxValue * scale) *
      Math.cos(radial_calcs.angleSlice * index - Math.PI / 2)
    );
  }
  ...
  function calcY(value, scale, index) {
    return (
      radial_calcs.rScale(value ? value : radial_calcs.maxValue * scale) *
      Math.sin(radial_calcs.angleSlice * index - Math.PI / 2)
    );
  }
  ...
  function setColor(d, index, key) {
    ...
    key = key ? key : d.key;
    return options.areas.colors[key]
      ? options.areas.colors[key]
      : options.color(index);
  }
  ...
  chart.updateDimensions = function() {
    if (typeof updateDimensions === "function")
      updateDimensions(transition_time);
  };
  ...
  chart.margins = function(value) {
    ...
    for (var k = 0; k < vKeys.length; k++) {
      if (mKeys.indexOf(vKeys[k]) >= 0)
        options.margins[vKeys[k]] = values[vKeys[k]];
    }
    return chart;
  };
  ...
  chart.ranges = function(values) {
    ...
    function checkRange(range_declarations) {
      ...
      for (var k = 0; k < keys.length; k++) {
        if (
          axes.indexOf(keys[k]) >= 0 && // is valid axis
  ...
  function tooltip_show(d, i, self) {
    ...
    let newX = parseFloat(d3.select(self).attr("cx")) - 10;
    let newY = parseFloat(d3.select(self).attr("cy")) - 10;
